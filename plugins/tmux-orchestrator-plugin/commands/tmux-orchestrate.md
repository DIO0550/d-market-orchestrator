...
**自律実行モード**: Phase 1〜3 はユーザー確認なしで自動完了する。ユーザーへの確認が必要な場合は Question 系ツールのみ使用し、それ以外では中断しない。

### 自動実行フェーズ（Phase 0-3）

このコマンドは以下のフェーズを自動で実行し、検証完了後に停止する：
...
```
1. プロジェクトタイプを自動検出してテスト・Lintコマンドを決定
2. test-runner と linter のプロンプトを生成
3. tmux ペインで並列起動・完了待機
4. .done の状態値を確認:
   a. 両方 PASS → Step 8 へ
   b. いずれか FAIL → debugger 起動 → マーカー削除 → 再実行（最大10回）
```

### Step 8: Phase 3 完了・報告
